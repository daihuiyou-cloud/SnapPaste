...
- 快速动作：选区完成后可复制、贴图、保存或 OCR。
- 贴图窗口：支持从截图或剪贴板创建置顶贴图，并可移动、缩放、旋转、翻转、调整透明度、点击穿透、复制、保存，以及内联标注编辑。
- 标注编辑：支持基础标注工具，包括画笔、箭头、矩形、马赛克和文字，通过**浮动工具栏**操作。
...
| 进入/退出编辑模式 | 工具栏按钮 |
| （编辑模式）撤销 | `Ctrl+Z` |
| （编辑模式）重做 | `Ctrl+Shift+Z` |
| （编辑模式）删除选中标注 | `Delete` |
| （编辑模式）退出编辑 | `Esc` / 浮动工具栏「Done」按钮 |
| （编辑模式）拖动浮动工具栏 | 鼠标拖拽工具栏任意空白区域 |
...
src/
  ...
  presentation/           Qt Widgets UI
    ...
    main_window/          主窗口
    ocr/                  OCR 结果窗口
    pin_window/           贴图窗口（拖拽、缩放、旋转、工具栏、OCR 覆盖层、内联标注编辑、浮动编辑工具栏）
    settings/             设置面板
    toast/                Windows 通知
    tray/                 系统托盘
...
- **内联标注编辑**：点击工具栏编辑按钮进入编辑模式，直接在贴图上绘制标注（画笔、箭头、矩形、文字、马赛克等）。复用 `AnnotationToolManager` 和 `AnnotationRenderer` 管理标注状态和渲染。编辑工具栏为**独立浮动窗口**（`EditToolbarWidget`），位于贴图窗口正下方，可鼠标拖拽移动，跟随贴图窗口移动。确认编辑后将标注渲染到原图，取消则丢弃。
...
- **贴图内联编辑**：编辑功能从独立 EditorWindow 移入 PinWindow（Snipaste 风格）。进入编辑模式后可直接在贴图上绘制标注，工具栏切换为编辑工具面板（矩形、箭头、画笔、文字、马赛克等），支持撤销/重做和层顺序调整。移除截图浮层动作栏中的编辑按钮和 Space 快捷键。
- **浮动编辑工具栏**：内联编辑的工具栏从窗口内嵌绘制改为**独立浮动窗口**（`EditToolbarWidget`），位于贴图窗口正下方，支持鼠标拖拽移动，跟随贴图窗口移动。按钮风格为图标+文字（与 `CaptureActionBar` 一致），工具分三组排列（绘图工具 | 撤销/重做 | 完成）。
