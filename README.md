...
src/
  ...
  presentation/           Qt Widgets UI
    ...
    main_window/          主窗口
    ocr/                  OCR 结果窗口
    pin_window/           贴图窗口（拖拽、缩放、旋转、工具栏、OCR 覆盖层、内联标注编辑）
    settings/             设置面板
    toast/                Windows 通知
    tray/                 系统托盘
...
- **内联标注编辑**：点击工具栏编辑按钮进入编辑模式，直接在贴图上绘制标注（画笔、箭头、矩形、文字、马赛克等）。复用 `AnnotationToolManager` 和 `AnnotationRenderer` 管理标注状态和渲染。确认编辑后将标注渲染到原图，取消则丢弃。
