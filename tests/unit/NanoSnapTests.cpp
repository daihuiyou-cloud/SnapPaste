...
class NanoSnapTests final : public QObject {
    Q_OBJECT
...
private slots:
    ...
    void jsonSettingsRepositoryReportsWriteFailure()
    {
        ...
        blocker.write("block");
        blocker.close();
        qputenv("NANOSNAP_DATA_DIR", blockerPath.toLocal8Bit());
    ...
    void sqliteHistoryRepositoryAddsListsAndDeletesRecords()
    {
        ...
        SqliteHistoryRepository repository(temporaryDir.filePath("history.sqlite"));
    ...
    void pinnedImageServiceCreatesFromImage()
    {
        ...
        SqlitePinnedItemRepository repository(temporaryDir.filePath("history.sqlite"));
    ...
    void pinnedImageStateIsClamped()
    {
        ...
        QCOMPARE(normalized.size.width(), 24);
        QCOMPARE(normalized.size.height(), 24);
    ...
    void localImageStorageUsesUniqueFileNames()
    {
        QTemporaryDir temporaryDir;
        QVERIFY(temporaryDir.isValid());
        qputenv("NANOSNAP_DATA_DIR", temporaryDir.path().toLocal8Bit());
...
QTEST_MAIN(NanoSnapTests)

#include "NanoSnapTests.moc"
